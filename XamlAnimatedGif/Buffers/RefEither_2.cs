...
namespace XamlAnimatedGif.Buffers
{
    ...
    public readonly ref struct RefEither<T1, T2> where T1 : allows ref struct where T2 : allows ref struct
    {
    	...
    	/// <summary>
    	/// Implicitly converts a value of type <typeparamref name="T1"/> to a <see cref="RefEither{T1, T2}"/> instance representing the first value.
    	/// </summary>
    	/// <remarks>This operator allows seamless assignment of a <typeparamref name="T1"/> value to a <see cref="RefEither{T1, T2}"/> without explicit
    	/// casting. The resulting <see cref="RefEither{T1, T2}"/> will contain the first value, and the second value will be
    	/// unset.</remarks>
    	/// <param name="first">The value of type <typeparamref name="T1"/> to be converted into a <see cref="RefEither{T1, T2}"/>.</param>
    	[DebuggerStepThrough] public static implicit operator RefEither<T1, T2>(T1 first) => new(first);
    	/// <summary>
    	/// Implicitly converts a value of type <typeparamref name="T2"/> to a <see cref="RefEither{T1, T2}"/> instance representing the second value.
    	/// </summary>
    	/// <remarks>This operator allows seamless assignment of a <typeparamref name="T2"/> value to a <see cref="RefEither{T1, T2}"/> without explicit
    	/// casting. The resulting <see cref="RefEither{T1, T2}"/> will represent the second value. This can simplify code when working with
    	/// APIs that accept <see cref="RefEither{T1, T2}"/>.</remarks>
    	/// <param name="second">The value of type <typeparamref name="T2"/> to be wrapped as the second alternative in the <see cref="RefEither{T1, T2}"/>.</param>
    	[DebuggerStepThrough] public static implicit operator RefEither<T1, T2>(T2 second) => new(second);
    }
}